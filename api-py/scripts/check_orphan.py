...
口径(唯一真源):
  1. 候选表**动态发现**:information_schema 中 COLUMN_NAME='merchant_id' 的表(禁止硬编码表名清单);
...
SQL_TABLE_DISCOVERY = (
    ...
    "WHERE TABLE_SCHEMA = DATABASE() AND COLUMN_NAME = 'merchant_id' ORDER BY TABLE_NAME"
)
...
SQL_TABLE_NULL_MID = "SELECT COUNT(*) AS c FROM `{table}` WHERE merchant_id IS NULL"
SQL_ORPHAN_GROUPS = (
    "SELECT x.merchant_id AS merchant_id, COUNT(*) AS row_count FROM `{table}` x "
    "LEFT JOIN merchant m ON m.merchant_id = x.merchant_id "
    "WHERE x.merchant_id IS NOT NULL AND m.id IS NULL GROUP BY x.merchant_id ORDER BY row_count DESC"
)
SQL_ORPHAN_SAMPLE_IDS = "SELECT id FROM `{table}` WHERE merchant_id = :mid ORDER BY id LIMIT {limit}"
...
def _read_tables(conn: Any) -> List[str]:
    from sqlalchemy import text

    return [row[0] for row in conn.execute(text(SQL_TABLE_DISCOVERY)).fetchall()]


def _scan_table(conn: Any, table: str, has_id: bool) -> Dict[str, Any]:
    from sqlalchemy import text

    total = conn.execute(text(SQL_TABLE_TOTAL.format(table=table))).scalar()
    null_mid = conn.execute(text(SQL_TABLE_NULL_MID.format(table=table))).scalar()
    groups = conn.execute(text(SQL_ORPHAN_GROUPS.format(table=table))).mappings().all()
    entries = aggregate_rows([(g["merchant_id"], g["row_count"]) for g in groups])
    for entry in entries.values():
        if has_id:
            rows = conn.execute(
                text(SQL_ORPHAN_SAMPLE_IDS.format(table=table, limit=5)), {"mid": entry["merchant_id"]}
            ).fetchall()
            entry["sample_ids"] = [r[0] for r in rows]
        else:
            entry["sample_ids"] = []
    outcome = classify(table, list(entries.values()))
    outcome.update({"table": table, "total_rows": int(total or 0), "null_mid": int(null_mid or 0)})
    return outcome
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        with engine.connect() as conn:
            ...
            if not tables:
                raise RuntimeError("information_schema 未返回任何含 merchant_id 列的表")
            id_tables = {row[0] for row in conn.execute(text(SQL_ID_COLUMN_DISCOVERY)).fetchall()}
            results = {t: _scan_table(conn, t, t in id_tables) for t in tables}
    ...
    print("扫描: " + str(len(tables)) + " 张含 merchant_id 列的表")
    ...
    for table in sorted(results):
        if results[table]["null_mid"]:
            print("  " + table + ".merchant_id IS NULL: " + str(results[table]["null_mid"]) + " 行")
